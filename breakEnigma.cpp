...
//alphabet table    
char numAlpha[26] = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'};

int alphaToNum(char inputLetter)
{
    for(int i = 0; i < 26; ++i)
    {
        if(inputLetter == numAlpha[i])
        {
            return i;
        }
    }
...
std::array<int, 3> rotate(std::array<int, 3> rotorState)
{
    //rotate and returnd
...
string enigma(string inputString, int rotorState1, int rotorState2, int rotorState3)
{
    //declare rotorstate
    std::array<int, 3> rotorState = {rotorState1, rotorState2, rotorState3};

    int inputStringNum[inputString.length()];
    for(int i = 0; i < inputString.length(); ++i)
    {
        inputStringNum[i] = alphaToNum(inputString[i]);
    }

    //calculate output with enigma
    string codes = "";
    for(int i = 0; i < inputString.length(); ++i)
    {
        ...
        {
            //calculate with enigma for inputString[i]
            int codeNum = forward(forward(forward(inputStringNum[i], rotor1, rotorState[0]), rotor2, rotorState[1]), rotor3, rotorState[2]);
            codeNum = reflector[codeNum];
            codeNum = backward(backward(backward(codeNum, rotor3I, rotorState[2]), rotor2I, rotorState[1]), rotor1I, rotorState[0]);

            codes += numAlpha[codeNum];

            rotorState = rotate(rotorState);
        }
    }
...
bool checkEnglish(string inputString)
{
    for(int i = 0; i < 10000; ++i)
    {
        if(inputString == dictionary[i])
        {
            return true;
        }
    }
...
int main()
{
    string breakCode = "gltayfacq vx uf faf wffvsx ch hgl goz bwrdumy tgrpnnul cz uch mj zmy zadtby tt ihc ezp nhcnkyo";
    string mytest = "hi my name is soichi";
    string myresult = enigma(mytest, 0, 0, 0);
    cout << myresult << endl;
    return 0;
